...
TagKit also has a few views that aim at making it easier to work with tags. For instance, ``TagTextField`` automatically slugifies text as you type, while ``TagList`` and ``TagEditList`` are used to list and edit tags, using custom tag view builders.
